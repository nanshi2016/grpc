...
class CFHostDNSResolverImpl
 ...
 public:
  ...
  void resolve(absl::string_view hostname) {
    CFTypeUniqueRef<CFStringRef> cf_hostname =
        CFStringCreateWithCString(nullptr, hostname, kCFStringEncodingUTF8);
    CFTypeUniqueRef<CFHostRef> cf_host =
        CFHostCreateWithName(nullptr, cf_hostname);

    CFHostClientContext cf_context = {0, this, Retain, Release, nullptr};
    CFHostSetClient(cf_host, CFHostCallback, &cf_context);
  }

 private:
  void CFHostCallback(CFHostRef theHost, CFHostInfoType typeInfo,
